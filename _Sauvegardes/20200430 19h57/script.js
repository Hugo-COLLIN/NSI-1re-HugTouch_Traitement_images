...
//var editZone = document.getElementById('editor-zone');

var x=0; //valeur du dernier filtre
//var x=1;
//var imageDataSource = ctx.getImageData(0, 0,img.width,img.height);

// Objet représentant un élément dans la pile de transformation
class stack {
  constructor(action) {
...
function apply(filter, value) {
    switch (filter){
        case 'bluring':
            actionsStack.push(new stack("bluring"));
            console.log("blur");
            //alert(lastAction = ACTIONS_STACK[ACTIONS_STACK.length-1])
            break;
        case 'brightness':
