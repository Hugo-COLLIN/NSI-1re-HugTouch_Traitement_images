...
function drawCanvas() {
    ctx.filter = `blur(${blur.value}px) brightness(${brightness.value}%) contrast(${contrast.value}%) grayscale(${grayscale.value}%) invert(${invert.value}%) saturate(${saturate.value}%) sepia(${sepia.value}%) hue-rotate(${hueRotate.value}deg)`;
