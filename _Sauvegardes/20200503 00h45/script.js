...
function apply(filter) {
    verifFilter = 0;
    /*if (filter != actionStack[actionStack.length-1]._action) {
        actionStack.push(new stack(filter));
        valueStack.push(new stack(valueBeforeFilter));
        console.log(actionStack[actionStack.length-1]._action + valueStack[valueStack.length-1]._action);
    }*/
    //for (let i=0; i<x; i++) {
    switch (filter){
...
function restore() {
    if (actionStack.length > 1){  // Si la pile contient des actions
        ...
        */
        if (actionStack[actionStack.length-1]._action != actionStack[actionStack.length-2]._action) {
            restore;
            //actionStack.pop();
            //valueStack.pop();
        }
