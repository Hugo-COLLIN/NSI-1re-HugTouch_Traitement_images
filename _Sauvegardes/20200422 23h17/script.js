...
function drawCanvas() {
    ctx.filter = `blur(${blur.value}px) brightness(${brightness.value}%) contrast(${contrast.value}%)`;
    //ctx.filter = `blur(${blur.value}px) brightness(${brightness.value}%) contrast(${contrast.value}% greyscale(${greyscale.value}% invert(${invert.value}% opacity(${opacity.value}% saturate(${saturate.value}% sepia(${sepia.value}%)`;
...
//greyscale.addEventListener('input',drawCanvas);
